...
df_sel <- df |> 
  select(Data, year, mes, all_of(vars)) |> 
  rename(valor = all_of(vars))
...
df_sel2 <- df_sel |> 
  group_by(year) |> 
  mutate( # if
         valor_z1 = scale(valor1)[,1],
         valor_z2 = scale(valor2)[,1],
         valor_z4 = scale(valor4)[,1],
         valor_z6 = scale(valor6)[,1],
...
plot1 <- df_sel3 |> 
  ...
  geom_hline(
    data = df_sel3 |> dplyr::distinct(var, linha_media),
